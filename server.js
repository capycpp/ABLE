...
function send(res,status,data,type='application/json',extra={}){res.writeHead(status,{'Content-Type':type,'Cache-Control':'no-store',...extra});res.end(type==='application/json'?JSON.stringify(data):data);}
...
async function api(req,res){
  ...
  if(req.method==='POST'&&p==='/api/signup'){
    try{const x=await body(req);if(!x.name||!x.email||!x.password||x.password.length<8)return send(res,400,{error:'Name, email and a password of at least 8 characters are required.'});
      ...
      const sid=crypto.randomBytes(32).toString('hex');sessions.set(sid,safeUser(user));return send(res,201,{user:safeUser(user)},'application/json',{'Set-Cookie':`able_session=${sid}; HttpOnly; Path=/; SameSite=Lax; Max-Age=604800`});
    }catch(e){return send(res,500,{error:'Could not create account.'});}
  }
  if(req.method==='POST'&&p==='/api/login'){
    try{const x=await body(req);const d=db();const user=d.users.find(v=>v.email.toLowerCase()===String(x.email||'').toLowerCase());if(!user||!(await verifyPassword(x.password||'',user.password)))return send(res,401,{error:'Invalid email or password.'});const sid=crypto.randomBytes(32).toString('hex');sessions.set(sid,safeUser(user));return send(res,200,{user:safeUser(user)},'application/json',{'Set-Cookie':`able_session=${sid}; HttpOnly; Path=/; SameSite=Lax; Max-Age=604800`});}catch(e){return send(res,500,{error:'Login failed.'});}
  }
  if(req.method==='POST'&&p==='/api/logout'){const sid=cookies(req).able_session;sessions.delete(sid);return send(res,200,{ok:true},'application/json',{'Set-Cookie':'able_session=; HttpOnly; Path=/; Max-Age=0; SameSite=Lax'});}
...
const server=http.createServer((req,res)=>req.url.startsWith('/api/')?api(req,res).catch(e=>send(res,500,{error:'Server error'})):serve(req,res));
